...
- **对话与任务闭环**：继续完善表单、附件预览、任务状态恢复和连接异常处理，并扩大真实渠道验收覆盖。
- **项目与编程工作流**：继续完善项目目录、工作树、Git 变更审核、终端与文件浏览体验，并补充系统级窗口回归。
- **通用浏览器能力**：继续扩大网页导航、页面感知与浏览器操作的真实环境覆盖。
- **Computer Use 稳定性**：完善多显示器、DPI、截图失败恢复、动作验证与执行效率。
- **扩展与设置**：完善插件、技能、MCP 的配置和生命周期管理，让设置项对应真实行为。
- **持续验收**：每实现一个功能单独提交，再进行验收、修正和回归提交。

目前仍有部分设置为展示项。附件选择已经接入 `turn/start`、`turn/steer` 和排队消息：图片作为 `localImage` 传给 app-server，其他文件作为带路径的文本输入，模型可按需使用文件工具读取；完整模型端到端效果仍取决于所选渠道。用户补充信息已支持阻塞式和非阻塞式表单、取消、重试及所属会话路由。本机截图曾出现 `CopyFromScreen：句柄无效`，仍需排查和回归。远程自动部署也不适用于所有 Linux 桌面环境。
