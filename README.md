...
项目目标是逐步接近 Codex Desktop 的功能与使用体验，构建集对话、编程、自动化、Computer Use 和扩展能力于一体的个人工作台。当前已经接入真实模型与工具执行链路，仍在持续开发和验收中，并非完整功能对等版本。
...
Felix 基于 **Codex app-server** 构建 Agent 执行能力，项目自身实现桌面交互、模型兼容适配、工具集成、远程环境准备和任务管理等功能。它不是从零重写的 Agent 推理引擎。
...
以“接近完整个人 Agent 工作台”为目标，逐项实现并验收：
