...
export type AppCommand = { id: string; label: string; keywords: string; run: () => void; disabled?: boolean };
export function CommandPalette({ commands, onClose }: { commands: AppCommand[]; onClose: () => void }) {
  ...
  return <dialog ref={dialog} className="command-palette" aria-label="命令面板" onCancel={event => { event.preventDefault(); onClose(); }}>
    ...
    <input ref={search} role="combobox" aria-label="搜索命令" aria-expanded="true" aria-controls="app-command-results" aria-activedescendant={results.length ? `app-command-${results[selected].id}` : undefined} placeholder="搜索操作，例如 Git、终端、设置…" value={query} onChange={event => { setQuery(event.target.value); setIndex(0); }} onKeyDown={event => {
    ...
    <div id="app-command-results" role="listbox" aria-label="可用命令">{results.map((command, position) => <button key={command.id} id={`app-command-${command.id}`} role="option" aria-selected={position === selected} aria-disabled={command.disabled || undefined} disabled={command.disabled} tabIndex={-1} onClick={() => execute(command)}>{command.label}</button>)}</div>
