...
export function AccountAuthPanel({ connected, busy, onChanged }: { connected: boolean; busy?: boolean; onChanged: () => void }) {
  const [mode, setMode] = useState<'chatgpt' | 'apiKey'>('chatgpt');
  ...
  return <section className="settings-card" aria-label="账户认证"><h2>账户认证</h2><p>登录状态可在上方账户信息中刷新查看。</p><fieldset disabled={!connected || working || busy} style={{ border: 0, padding: 0 }}><label>登录方式 <select aria-label="账户登录方式" value={mode} onChange={event => setMode(event.target.value as typeof mode)}><option value="chatgpt">ChatGPT 浏览器登录</option><option value="apiKey">API Key</option></select></label>{mode === 'apiKey' && <input aria-label="账户 API Key" type="password" autoComplete="off" value={apiKey} onChange={event => setApiKey(event.target.value)} placeholder="粘贴 API Key" />}{login ? <div><p role="status">请在浏览器完成登录{login.code ? `，验证码：${login.code}` : ''}</p>{login.url && <button type="button" onClick={() => void window.desktop?.openExternal?.(login.url!)}>打开登录页面</button>}<button type="button" onClick={() => void cancel()}>取消登录</button></div> : <button type="button" onClick={() => void start()}>{mode === 'apiKey' ? '保存 API Key' : '开始登录'}</button>}<button type="button" onClick={() => void logout()}>退出当前账户</button></fieldset>{message && <p role="status">{message}</p>}</section>;
}
