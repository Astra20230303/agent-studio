...
export function PreviewText({ text, lineNumber, truncated, ref }: { text: string; lineNumber?: number; truncated?: boolean; ref: Ref<PreviewTextHandle> }) {
  ...
  return <div className="preview-text">
    ...
    <pre ref={pre} tabIndex={0} aria-label="文件预览文本" style={{ overflow: 'auto', maxHeight: '55vh', whiteSpace: 'pre', margin: '12px 0' }}>{text.split('\n').map((value, index) => {
      ...
      return <span key={index} ref={index + 1 === lineNumber ? line : undefined} style={index + 1 === lineNumber ? { background: '#ffe08a', color: '#202020' } : undefined}>{highlighted ? <>{value.slice(0, from)}<mark ref={match.start >= start ? selected : undefined}>{value.slice(from, to)}</mark>{value.slice(to)}</> : value}{'\n'}</span>;
    })}</pre>
  </div>;
}
