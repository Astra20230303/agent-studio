...
export async function startTurn(params: { threadId: string; text: string; planningMode?: 'default' | 'plan'; plugins?: { id: string; name: string }[]; cwd?: string; model?: string; modelProvider?: string; effort?: string }) { return unwrap<any>(bridge().request('turn/start', { threadId: params.threadId, ...(params.planningMode && params.model ? { collaborationMode: collaborationMode(params.planningMode, params.model, params.effort) } : {}), input: [{ type: 'text', text: params.text }, ...(params.plugins || []).map(plugin => ({ type: 'mention', name: plugin.name, path: `plugin://${plugin.id}` }))], ...(params.cwd ? { cwd: params.cwd } : {}), ...(params.model ? { model: params.model } : {}), modelProvider: 'minimax', ...(params.effort ? { effort: params.effort } : {}) })); }
...
export async function steerTurn(threadId: string, expectedTurnId: string, text: string, plugins: { id: string; name: string }[] = []) {
  return unwrap<any>(bridge().request('turn/steer', { threadId, expectedTurnId, input: [{ type: 'text', text }, ...plugins.map(plugin => ({ type: 'mention', name: plugin.name, path: `plugin://${plugin.id}` }))] }));
}
export async function listThreads() { return unwrap<any>(bridge().request('thread/list', { limit: 100, sortKey: 'recencyAt', sortDirection: 'desc' })); }
