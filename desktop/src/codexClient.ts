...
import { readThreadDaybreakResponse, updateThreadDaybreakParams, updateThreadGitParams, updateThreadProjectParams } from './threadMetadata';
...
export async function readCodexProviderCapabilities(): Promise<CodexProviderCapabilities> { return parseCodexProviderCapabilities(await unwrap<unknown>(bridge().request('modelProvider/capabilities/read', {}))); }
export async function listRemoteQueuedSubmissions(threadId: string): Promise<RemoteQueuedSubmission[]> { const data: RemoteQueuedSubmission[] = []; let cursor: string | undefined; const seen = new Set<string>(); for (let page = 0; page < 100; page++) { const result = readRemoteQueuePage(await unwrap<unknown>(bridge().request('thread/queue/list', { ...remoteQueueIdentity(threadId), limit: 100, ...(cursor ? { cursor } : {}) }))); data.push(...result.data); if (!result.nextCursor) return data; if (seen.has(result.nextCursor)) throw new Error('服务端队列分页重复，请重试'); seen.add(result.nextCursor); cursor = result.nextCursor; } throw new Error('服务端队列页数过多'); }
export async function addRemoteQueuedSubmission(threadId: string, text: string) { if (!text.trim() || text.length > 100000) throw new Error('排队消息内容无效'); await unwrap(bridge().request('thread/queue/add', { ...remoteQueueIdentity(threadId), input: userInput(text), clientUserMessageId: crypto.randomUUID() })); }
export async function deleteRemoteQueuedSubmission(threadId: string, submissionId: string) { await unwrap(bridge().request('thread/queue/delete', remoteQueueIdentity(threadId, submissionId))); }
export async function startRemoteQueuedSubmission(threadId: string, submissionId?: string) { await unwrap(bridge().request('thread/queue/start', remoteQueueIdentity(threadId, submissionId))); }
...
export async function updateThreadDaybreak(threadId: string, enabled: boolean) { return readThreadDaybreakResponse(await unwrap<unknown>(bridge().request('thread/metadata/update', updateThreadDaybreakParams(threadId, enabled))), threadId); }
