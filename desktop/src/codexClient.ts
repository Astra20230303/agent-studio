...
export async function startTurn(params: { threadId: string; text: string; plugins?: { id: string; name: string }[]; cwd?: string; model?: string; modelProvider?: string; effort?: string }) { return unwrap<any>(bridge().request('turn/start', { threadId: params.threadId, input: [{ type: 'text', text: params.text }, ...(params.plugins || []).map(plugin => ({ type: 'mention', name: plugin.name, path: `plugin://${plugin.id}` }))], ...(params.cwd ? { cwd: params.cwd } : {}), ...(params.model ? { model: params.model } : {}), modelProvider: 'minimax', ...(params.effort ? { effort: params.effort } : {}) })); }
