...
export async function searchThreads(searchTerm: string, cursor?: string) { return unwrap<any>(bridge().request('thread/search', { searchTerm, limit: 100, sortKey: 'recency_at', sortDirection: 'desc', ...(cursor ? { cursor } : {}) })); }
