...
export type TaskDraft = { followupThreadId?: string; timeoutMinutes?: number; reasoningEffort?: ExplicitReasoningEffort; id?: string; providerId?: string; cwd?: string; name: string; prompt: string; kind: 'agent' | 'reminder'; model: string; permission: 'read-only' | 'workspace-write'; notify: boolean; notificationPolicy?: 'failed_runs_only' | null; schedule: TaskSchedule };
...
export type TaskRun = { silent?: boolean; outputTruncated?: boolean; environment?: { cwd: string; providerId?: string }; configuration?: TaskRunConfiguration; threadId?: string; id: string; status: 'running' | 'completed' | 'failed' | 'interrupted'; trigger: 'manual' | 'scheduled'; startedAt: string; finishedAt?: string; error?: string; output?: string };
