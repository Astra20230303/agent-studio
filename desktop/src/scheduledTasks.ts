...
export type TaskDraft = { id?: string; providerId?: string; cwd?: string; name: string; prompt: string; kind: 'agent' | 'reminder'; model: string; permission: 'read-only' | 'workspace-write'; notify: boolean; notificationPolicy?: 'failed_runs_only' | null; schedule: TaskSchedule };
