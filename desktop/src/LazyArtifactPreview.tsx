...
export function LazyArtifactPreview(props: ComponentProps<typeof ArtifactPreview>) {
  ...
  return Preview ? <Preview {...props} /> : <LoadingPreview docked={props.docked} failed={error} onClose={props.onClose} />;
}

function LoadingPreview({ failed, onClose, docked }: { docked?: boolean; failed: boolean; onClose: () => void }) {
  ...
  return <dialog ref={dialog} className={`file-editor${docked ? ' docked-file' : ''}`} aria-label="文件预览加载" onCancel={event => { event.preventDefault(); onClose(); }}>
