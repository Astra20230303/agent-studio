...
export type McpStatusPage = {
  data: Array<{ name: string; runtimeStatus?: string; authStatus: string; tools?: Record<string, { description?: string }>; toolsError?: string; resources?: McpResource[]; resourceTemplates?: McpResourceTemplate[] }>;
  nextCursor?: string;
};
...
export function parseMcpStatusPage(value: unknown): McpStatusPage {
  ...
  const data = value.data.map((server: any) => {
    if (!object(server) || typeof server.name !== 'string' || !server.name.trim() || names.has(server.name)
      ...
      || server.tools != null && (!object(server.tools) || Object.entries(server.tools).some(([name, tool]: any) => !name || !object(tool) || tool.description != null && typeof tool.description !== 'string'))
