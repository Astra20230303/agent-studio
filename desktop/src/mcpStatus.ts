...
export type McpToolInfo = { name?: string; annotations?: unknown; description?: string; inputSchema?: Record<string, unknown>; outputSchema?: Record<string, unknown> };
...
export function parseMcpStatusPage(value: unknown): McpStatusPage {
  ...
  const data = value.data.map((server: any) => {
    if (!object(server) || typeof server.name !== 'string' || !server.name.trim() || names.has(server.name)
      ...
      || server.tools != null && (!object(server.tools) || Object.entries(server.tools).some(([name, tool]: any) => !name || !object(tool) || tool.name != null && (typeof tool.name !== 'string' || !tool.name.trim()) || tool.description != null && typeof tool.description !== 'string' || tool.inputSchema != null && !object(tool.inputSchema) || tool.outputSchema != null && !object(tool.outputSchema)))
