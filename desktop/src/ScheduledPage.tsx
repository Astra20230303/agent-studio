...
export function ScheduledPage({ providers, cwd, openRequest, onOpenHandled }: { onOpenHandled?: (request: { id: string }) => void; openRequest?: { id: string }; providers: { id: string; name: string; enabled?: boolean }[]; cwd?: string }) {
  ...
  return <div className="scheduled-page">
    ...
    {selectedId && <TaskModal title={detail?.name || '任务详情'} onClose={() => { setSelectedId(undefined); setDetail(undefined); }}>{detailError && <div className="task-error" role="alert"><span>{detailError}</span><button onClick={() => void reload()}>重试</button></div>}{!detail ? !detailError && <p role="status">正在读取记录…</p> : <>
      ...
      <h3>运行记录</h3><div className="task-runs">{detail.runs.length ? detail.runs.map(run => <details key={run.id} className="task-run"><summary><span>{formatTaskDate(run.startedAt)}</span><span className={run.status === 'failed' ? 'task-failed' : ''}>{taskRunLabels[run.status]}</span></summary><small>{run.trigger === 'scheduled' ? '定时执行' : '手动执行'}{run.finishedAt ? ` · 结束于 ${formatTaskDate(run.finishedAt)}` : ''}</small>{run.error && <p className="task-failed">{run.error}</p>}<pre>{run.output || (run.status === 'running' ? '正在执行，结束后保存结果。' : '无输出')}</pre><TaskRunActions key={`${run.id}:${run.status}`} name={detail.name} run={run} /></details>) : <p className="task-muted">尚无运行记录</p>}</div>
