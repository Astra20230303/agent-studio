...
function TaskEditor({ draft, providers, onClose, onSaved, onChange }: { draft: TaskDraft; providers: { id: string; name: string; enabled?: boolean }[]; onClose: () => void; onSaved: () => void; onChange: (draft: TaskDraft) => void }) {
...
export function ScheduledPage({ providers, cwd, openRequest, onOpenHandled, onOpenConversation, onRecord }: { onRecord?: (action: string) => void; onOpenConversation?: (threadId: string, title: string) => void; onOpenHandled?: (request: { id: string }) => void; openRequest?: { id: string }; providers: { id: string; name: string; enabled?: boolean }[]; cwd?: string }) {
  ...
  return <div className="scheduled-page">
    ...
    {draft && <TaskEditor draft={draft} providers={providers} onClose={() => { setDraft(undefined); setSavedDraft(undefined); }} onChange={setSavedDraft} onSaved={() => { onRecord?.(draft.id ? '编辑任务' : '创建任务'); setDraft(undefined); setSavedDraft(undefined); void reload(); }} />}
