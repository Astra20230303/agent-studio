    ...
    <fieldset disabled={saving}>
      ...
      <div className="task-form-grid"><label>类型<select value={form.kind} onChange={event => patch({ kind: event.target.value as TaskDraft['kind'] })}><option value="agent">Agent 任务</option><option value="reminder">提醒</option></select></label>
        <label>频率<select value={form.schedule.kind} onChange={event => {
          ...
          patch({ schedule: kind === 'interval' ? { kind, minutes: 60 } : kind === 'once' ? { kind, at: new Date(Date.now() + 3600000).toISOString() } : { time: '09:00', timezone: localZone, ...(form.schedule.kind !== 'once' && form.schedule.kind !== 'interval' ? form.schedule : {}), kind, ...(kind === 'customWeek' ? { days: form.schedule.kind === 'customWeek' ? form.schedule.days : [1, 3, 5] } : {}), ...(kind === 'weekly' ? { day: form.schedule.kind === 'weekly' ? form.schedule.day : 1 } : {}) } });
        }}><option value="interval">固定间隔</option><option value="daily">每天</option><option value="weekdays">工作日</option><option value="weekly">每周</option><option value="customWeek">每周多日</option><option value="once">仅一次</option></select></label></div>
