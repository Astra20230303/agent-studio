...
export function TurnQueue({ items, onRemove, onResume, disabled }: { items: QueuedTurn[]; onRemove: (id: string) => void; onResume: () => void; disabled: boolean }) {
  ...
  return <section className="turn-queue" aria-label="待发送消息"><header>待发送 · {items.length}{items.some(item => item.status === 'paused') && <button disabled={disabled} onClick={onResume}>继续队列</button>}</header><ol>{items.map(item => <li key={item.id}><span>{item.text}<small>{item.error || (item.status === 'sending' ? '正在发送…' : '本轮完成后发送')}</small></span><button disabled={item.status === 'sending'} aria-label={`取消排队：${item.text}`} onClick={() => onRemove(item.id)}>取消</button></li>)}</ol></section>;
}
