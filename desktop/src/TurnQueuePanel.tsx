...
type QueueConfiguration = Pick<QueuedTurn, 'model' | 'effort' | 'planningMode'>;
...
function QueueEditor({ item, onClose, onSave, catalog }: { catalog?: QueueCatalog; item: QueuedTurn; onClose: () => void; onSave: (text: string, attachments: string[], configuration: QueueConfiguration) => boolean }) {
  ...
  return <dialog ref={dialog} className="file-editor" aria-label="编辑排队消息" onCancel={event => { event.preventDefault(); onClose(); }}>
    ...
    <button onClick={onClose}>取消编辑</button><button disabled={Boolean(invalidConfiguration) || !text.trim() && !attachments.length && !item.skills?.length} onClick={() => { if (invalidConfiguration) return; if (onSave(text.trim(), attachments, { model, effort, planningMode })) onClose(); else setError('消息未保存，请检查队列状态后重试。'); }}>保存排队消息</button>
  </dialog>;
}
