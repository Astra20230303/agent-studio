...
export function taskRunExport(name: string, run: TaskRun): string {
  ...
  if (config) {
    ...
    if (config.kind === 'agent') lines.push(`渠道配置：${config.providerId || '跟随运行时启用渠道'}`, `模型：${config.model}`, `推理强度：${config.reasoningEffort || '模型默认'}`, `目录配置：${config.cwd || '使用运行时默认项目目录'}`, `权限：${config.permission}`);
