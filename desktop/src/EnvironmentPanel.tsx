...
import { readEnvironmentInfo as requestInfo, readEnvironmentStatus as requestStatus } from './codexClient';
...
export function EnvironmentPanel({ connected, busy }: { connected: boolean; busy?: boolean }) {
  ...
  return <section className="settings-card" aria-label="Codex 执行环境"><h2>Codex 执行环境</h2><p>查看当前 app-server 配置的执行环境和 shell 信息。</p><label>环境 ID<input value={environmentId} onChange={event => setEnvironmentId(event.target.value)} onKeyDown={event => { if (event.key === 'Enter') void load(); }} /></label><button disabled={!connected || busy || loading} onClick={() => void load()}>{loading ? '读取中…' : '读取环境'}</button>{status && <p role="status">连接状态：{labels[status.status]}{status.error ? ` · ${status.error}` : ''}</p>}{info && <dl><dt>Shell</dt><dd>{info.shell.name} · {info.shell.path}</dd>{info.cwd && <><dt>默认工作目录</dt><dd>{info.cwd}</dd></>}</dl>}{notice && <p role="alert">{notice}</p>}</section>;
}
