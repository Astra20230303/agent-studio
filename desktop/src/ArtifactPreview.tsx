...
export function ArtifactPreview({ target, onClose, onEdit, docked = false }: { docked?: boolean; target: ArtifactTarget; onClose: () => void; onEdit: (session: FileEditSession) => void }) {
  ...
  return <dialog ref={dialog} className={`file-editor file-preview${docked ? ' docked-file' : ''}`} aria-label="消息文件预览" onKeyDown={event => {
    ...
    <div><button onClick={() => setRevision(value => value + 1)}>刷新预览</button>{isEditablePreview(preview) && <button onClick={() => onEdit({ root: target.root, path: target.path, ...textPreview.current?.currentPosition(), initial: { text: preview.text, revision: preview.revision } })}>编辑此文件</button>}<button onClick={onClose}>关闭预览</button></div>
  </dialog>;
}
