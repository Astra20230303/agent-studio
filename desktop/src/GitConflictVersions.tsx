...
export function GitConflictVersions({ root, path }: { root: string; path: string }) {
  ...
  return <section aria-label="冲突版本"><h3>冲突版本</h3><p>变基时，第 2 阶段通常是目标分支，第 3 阶段是正在重放的提交。</p>{error ? <p role="alert">{error}<button onClick={() => setRevision(value => value + 1)}>重试读取版本</button></p> : !stages ? <p role="status">正在读取冲突版本…</p> : !stages.length ? <p role="status">该文件没有未合并版本，冲突可能已解决。请返回并刷新 Git 变更。</p> : ['共同基线（第 1 阶段）', '当前侧（第 2 阶段）', '传入侧（第 3 阶段）'].map((label, index) => {
