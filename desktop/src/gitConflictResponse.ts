...
export function parseConflictStages(value: unknown) {
  if (!object(value) || !Array.isArray(value.stages) || value.stages.some((stage: any) => !object(stage) || ![1, 2, 3].includes(stage.stage)
    ...
    || stage.text != null && stage.unavailable != null
    || typeof stage.text !== 'string' && (typeof stage.unavailable !== 'string' || !stage.unavailable.trim())) || new Set(value.stages.map((stage: any) => stage.stage)).size !== value.stages.length) throw Error('冲突版本数据无效，请重试');
