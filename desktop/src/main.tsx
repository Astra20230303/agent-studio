...
function App() {
  ...
  return <div className={`desktop-app ${state.theme} ${page === 'settings' ? 'settings-mode' : ''} ${terminalOpen ? 'terminal-visible' : ''}`}>
    <header className="desktop-titlebar">
      ...
      <button aria-label="浏览工作区文件" aria-expanded={filesOpen} onClick={() => setFilesOpen(value => !value)}><FolderOpen size={17} /></button><button aria-label="查看 Git 变更" aria-expanded={gitOpen} onClick={() => { setGitOpen(value => !value); setFilesOpen(false); }}><GitBranch size={17} /></button><button aria-label="打开终端" title="终端" aria-expanded={terminalOpen} onClick={() => { if (!terminalStarted) { setTerminalCwd(workspaceFor(state, active)); setTerminalStarted(true); } setTerminalOpen(value => !value); }}><Terminal size={17} /></button><nav aria-label="应用菜单"><button>文件</button><button>编辑</button><button>视图</button><button>帮助</button></nav><button className="remote-browser-toggle" aria-label="浏览器" title="浏览器" aria-expanded={browserOpen} aria-controls="remote-browser" onClick={() => setBrowserOpen(value => !value)}><Globe size={17} /></button><WindowControls />
    </header>
