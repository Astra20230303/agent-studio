...
function App() {
  ...
  const enqueue = () => {
    ...
    if (!queue.change(items => [...items, { id: crypto.randomUUID(), localId: active.id, threadId: active.remoteId!, text: input.trim(), attachments: [...attachments], skills: [...composerSkills], model: modelId(activeModel), effort: state.reasoningEffort, cwd: workspaceFor(state, active), planningMode: active.planningMode || 'default', plugins: composerPlugins.map(({ id, name }) => ({ id, name })), waitingOn: runningTurnId, status: 'waiting' }], true)) return;
  ...
  const send = async () => {
    ...
    try {
      ...
      update(next => { const thread = next.threads.find(item => item.id === localId); if (thread) thread.model = model; });
  ...
  <div className="composer-dock">
    ...
    <div className="composer">
      ...
      <div className="composer-footer">
        ...
        <div className="composer-right"><EffortPicker model={model} value={reasoningEffort} onChange={value => update(next => { next.reasoningEffort = value; })} /><ModelPicker catalog={catalog} selected={model} open={showModel} setOpen={setShowModel} onSelect={id => update(next => { const thread = next.threads.find(item => item.id === active?.id); if (thread) thread.model = id; else next.model = id; })} />{running && <button className="send" title="停止生成" aria-label="停止生成" onClick={cancel}><Square aria-hidden="true" /></button>}<button className="send" title={running ? '追加指令' : '发送'} aria-label={running ? '追加指令' : '发送'} disabled={!canSend} onClick={send}><ArrowUp aria-hidden="true" /></button></div>
      </div>
    </div>
  </div></div>;
}
