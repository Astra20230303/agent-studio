...
import { CodexModelCatalogPanel } from './CodexModelCatalogPanel';
import { CodexProviderCapabilitiesPanel } from './CodexProviderCapabilitiesPanel';
...
function App({ initialState }: { initialState: DesktopState }) {
  ...
  const openSidePanel = (panel: 'files' | 'git' | 'agents' | 'diff') => {
    if (fileEdit) { setNotice('请先保存或取消当前文件编辑，再切换面板。'); return; }
    setArtifactTarget(undefined); setFilesOpen(panel === 'files'); setGitOpen(panel === 'git'); setAgentsOpen(panel === 'agents'); setDiffOpen(panel === 'diff');
    document.querySelectorAll<HTMLElement>('[popover]:popover-open').forEach(element => element.hidePopover());
  };
  ...
  useAppShortcuts({
    files: () => { openSidePanel('files'); setGitOpen(false); requestAnimationFrame(() => document.querySelector<HTMLInputElement>('[aria-label="查找工作区文件"]')?.focus()); },
    ...
    void (async () => {
      try {
        ...
        update(next => {
          ...
          thread.daybreakEnabled = loaded.daybreakEnabled;
  ...
  const revertFromMessage = async (messageId: string) => {
    ...
    try {
      ...
      update(next => { const target = next.threads.find(item => item.id === thread.id); if (target) { target.messages = restoreMessages(items, []); target.status = 'completed'; delete target.plan; delete target.planDelta; delete target.turnDiff; delete target.moderationMetadata; } });
  ...
  const feedbackControl = <><FeedbackPanel connected={codexStatus === 'connected'} busy={serviceInUse} threadId={active?.remoteId} /><RemoteProjectsPanel connected={codexStatus === 'connected'} busy={serviceInUse} threadId={active?.remoteId} /><RemoteControlPanel connected={codexStatus === 'connected'} busy={serviceInUse} /><EnvironmentPanel connected={codexStatus === 'connected'} busy={serviceInUse} /><CodexModelCatalogPanel connected={codexStatus === 'connected'} busy={serviceInUse} /><CodexProviderCapabilitiesPanel connected={codexStatus === 'connected'} busy={serviceInUse} /></>;
  return <div className={`desktop-app ${effectiveTheme} ${page === 'settings' ? 'settings-mode' : ''} ${terminalOpen ? 'terminal-visible' : ''}`}>
    ...
    {paletteOpen && <CommandPalette onClose={() => setPaletteOpen(false)} commands={[
      ...
      { id: 'files', label: '浏览工作区文件', keywords: 'files workspace', run: () => { openSidePanel('files'); setGitOpen(false); } },
      { id: 'git', label: '查看 Git 变更', keywords: 'diff commit branch', run: () => { openSidePanel('git'); setFilesOpen(false); } },
    ...
    <header className="desktop-titlebar">
      ...
      <button aria-label="浏览工作区文件" aria-expanded={filesOpen} onClick={() => filesOpen ? setFilesOpen(false) : openSidePanel('files')}><FolderOpen size={17} /></button><button aria-label="查看 Git 变更" aria-expanded={gitOpen} onClick={() => { gitOpen ? setGitOpen(false) : openSidePanel('git'); setFilesOpen(false); }}><GitBranch size={17} /></button><button aria-label="打开终端" title="终端" aria-expanded={terminalOpen} onClick={() => { if (!terminalStarted) { setTerminalStarted(true); } setTerminalOpen(value => !value); }}><Terminal size={17} /></button><button aria-label="打开命令面板" title="命令面板（Ctrl/⌘+Shift+P）" onClick={() => setPaletteOpen(true)}>命令</button><AppMenus groups={[
        { label: '文件', actions: [
          { label: '新建会话', run: newChat },
          { label: '浏览工作区文件', run: () => { openSidePanel('files'); setGitOpen(false); } },
        ...
        { label: '视图', actions: [
          ...
          { label: '查看 Git 变更', run: () => { openSidePanel('git'); setFilesOpen(false); } },
    ...
    <div className="desktop-body"><aside id="workspace-sidebar" className="sidebar" aria-label="侧栏" hidden={!sidebarVisible}>
      ...
      {gitOpen && !artifactTarget && !fileEdit && <GitPanel threadId={active?.remoteId} onRecord={audit.record} protectedPaths={state.threads.filter(thread => thread.status === 'running' || pendingThreads.includes(thread.id) || thread.remoteId && (restoringThread === thread.remoteId || runtime.threads[thread.remoteId]?.turnId) || queue.items.some(item => item.localId === thread.id)).map(thread => workspaceFor(state, thread)).filter((path): path is string => Boolean(path))} onReview={text => { setInput(current => current ? `${current}\n\n${text}` : text); setPage('chat'); setGitOpen(false); }} key={workspaceFor(state, active) || 'none'} root={workspaceFor(state, active)} onClose={() => setGitOpen(false)} onWorktree={project => { changeProject(project); setGitOpen(false); }} />}
      {filesOpen && !artifactTarget && !fileEdit && <WorkspaceFiles onPreview={setArtifactTarget} onEdit={setFileEdit} previewUpdate={filePreviewUpdate} key={workspaceFor(state, active) || 'none'} root={workspaceFor(state, active)} onClose={() => setFilesOpen(false)} onAttach={path => setAttachments(current => [...new Set([...current, path])])} />}
      {agentsOpen && !artifactTarget && !fileEdit && <AgentWorkspacePanel key={active?.id || 'new'} threads={state.threads} active={active} connected={codexStatus === 'connected'} onOpen={openAgent} onClose={() => setAgentsOpen(false)} />}
      {diffOpen && !artifactTarget && !fileEdit && <aside className="artifact-side-panel" aria-label="本回合差异面板"><button onClick={() => setDiffOpen(false)}>关闭差异面板</button><TurnDiffPanel value={active?.turnDiff} />{!active?.turnDiff && <p>当前会话没有回合差异。</p>}</aside>}
      {(artifactTarget || fileEdit) && <aside className="artifact-side-panel" aria-label="文件面板">    {artifactTarget && !fileEdit && <ArtifactPreview docked key={`${artifactTarget.root}:${artifactTarget.path}`} target={artifactTarget} onClose={() => setArtifactTarget(undefined)} onEdit={session => { setArtifactTarget(undefined); setFileEdit(session); }} />}
    {fileEdit && <FileEditor docked column={fileEdit.column} matchLength={fileEdit.matchLength} lineNumber={fileEdit.lineNumber} root={fileEdit.root} path={fileEdit.path} initial={fileEdit.initial} onClose={() => setFileEdit(undefined)} onSaved={preview => setFilePreviewUpdate({ root: fileEdit.root, path: fileEdit.path, preview })} />}
</aside>}
...
function Chat({ compaction, stopping, savingImages, onPasteImages, onDropAttachments, loadFullHistory, onChangePermission, onRetryFailure, sendShortcut, composerSkills, setComposerSkills, onOpenAgent, removeAttachment, busy, mode, permission, onOpenPlugins, composerPlugins, setComposerPlugins, onForkMessage, onRevertMessage, catalog, active, input, setInput, send, cancel, running, activity, model, reasoningEffort, update, attachments, addAttachment, showModel, setShowModel, showProjects, setShowProjects, toast, projectId, projects, status, onProjectChange, onRemoveProject }: { onRemoveProject: (id: string) => void; compaction: ReturnType<typeof useContextCompaction>; stopping: boolean; loadFullHistory: (options?: HistoryReadOptions) => Promise<void>; onChangePermission: (permission: DesktopState['permission']) => Promise<void>; onRetryFailure: (messageId: string) => void; sendShortcut?: 'enter' | 'mod-enter'; composerSkills: SelectedSkill[]; setComposerSkills: (skills: SelectedSkill[]) => void; onOpenAgent: (id: string) => void; removeAttachment: (path: string) => void; busy: boolean; mode: DesktopState['mode']; permission: DesktopState['permission']; onOpenPlugins: () => void; composerPlugins: Plugin[]; setComposerPlugins: (plugins: Plugin[]) => void; onForkMessage: (messageId: string) => Promise<void>; onRevertMessage: (messageId: string) => Promise<void>; catalog: ReturnType<typeof useModelCatalog>; active: DesktopState['threads'][number] | undefined; input: string; setInput: (value: string) => void; send: () => void; cancel: () => void; running: boolean; activity?: string; model: string; reasoningEffort: DesktopState['reasoningEffort']; update: (fn: (next: DesktopState) => void) => void; savingImages: boolean; onPasteImages: (event: ClipboardEvent) => void; onDropAttachments: (paths: string[]) => void; attachments: string[]; addAttachment: () => void; showModel: boolean; setShowModel: (value: boolean) => void; showProjects: boolean; setShowProjects: (value: boolean) => void; toast: (text: string) => void; projectId?: string; projects: DesktopState['projects']; status: string; onProjectChange: (project: Project) => void }) {
  ...
  return <div className={`chat-layout${workMode ? ' work-mode' : ''}${workMode && empty ? ' work-new-chat' : ''}`}>{active && <ConversationFind searchRemote={active.remoteId && status === 'connected' ? (term, signal) => searchThreadOccurrences(active.remoteId!, term, signal) : undefined} reset={findReset} key={active.id} messages={active.messages} view={threadView} searching={searchingConversation} loadHistory={active.remoteId && status === 'connected' ? loadFullHistory : undefined} disabled={busy || running} />}{active && (active.messages.length > 0 || activity) ? <div className="thread-view" ref={threadView}>{groupMessages(active.messages).map(group => {
  ...
  })}{active?.moderationMetadata && <details className="moderation-metadata"><summary>本回合审核元数据 · {active.moderationMetadata.turnId}</summary><pre>{JSON.stringify(active.moderationMetadata.metadata, null, 2)}</pre></details>}{activity && <div className={`activity${activity === '正在思考…' ? ' thinking' : ''}`}>{activity}</div>}</div> : !workMode && <div className="welcome">
