...
function App({ initialState }: { initialState: DesktopState }) {
  ...
  useEffect(() => {
    ...
    return desktop?.onOpenConversation?.(threadId => {
      ...
      setPage('chat'); setSidebarVisible(true);
    });
  }, []);
  ...
  <div className="composer-dock">
    ...
    <div className="composer" {...fileDrop} onPaste={onPasteImages}>
      ...
      <div className="composer-footer">
        ...
        <div className="composer-right"><EffortPicker model={model} value={reasoningEffort} onChange={value => update(next => { const thread = next.threads.find(item => item.id === active?.id); if (thread) thread.reasoningEffort = value; else next.reasoningEffort = value; })} /><ModelPicker catalog={catalog} selected={model} open={showModel} setOpen={setShowModel} onSelect={id => update(next => { const thread = next.threads.find(item => item.id === active?.id); if (thread) thread.model = id; else next.model = id; })} />{running && <button className="send" title="停止生成" aria-label="停止生成" aria-busy={stopping} disabled={stopping} onClick={cancel}><Square aria-hidden="true" /></button>}<button className="send" title={running ? '追加指令' : '发送'} aria-label={running ? '追加指令' : '发送'} disabled={!canSend} onClick={send}><ArrowUp aria-hidden="true" /></button></div>
      </div>
    </div>
  </div></div>;
}

function Workspace({ audit, serviceControl, page, state, models, update, toast, providerStatus, onBack }: { audit: ReturnType<typeof useAuditLog>; serviceControl: ReactNode; page: Page; state: DesktopState; models: string[]; update: (fn: (next: DesktopState) => void) => void; toast: (text: string) => void; providerStatus?: any; onBack: () => void }) {
  if (page === 'settings') return <SettingsWorkspace audit={audit} serviceControl={serviceControl} state={state} update={update} toast={toast} onBack={onBack} />;
...
function SettingsWorkspace({ audit, serviceControl, state, update, toast, onBack }: { audit: ReturnType<typeof useAuditLog>; serviceControl: ReactNode; state: DesktopState; update: (fn: (next: DesktopState) => void) => void; toast: (text: string) => void; onBack: () => void }) {
