...
import { clearThreadGoal, connectCodex, createThreadSection, deleteThreadSection, getThreadGoal, listThreadSections, moveThreadSection, readAccount, readAccountRateLimits, readAccountTokenUsage, resetMemory, revertThread, setThreadGoal, setThreadMemoryMode, startReview, subscribeCodex, updateThreadSection } from './codexClient';
...
function App({ initialState }: { initialState: DesktopState }) {
  ...
  const updateSection = async (id: string, name: string) => { try { const section = await updateThreadSection(id, name); setThreadSections(current => current.map(item => item.id === id ? section : item)); toast('分组名称已更新'); } catch (error) { toast(`更新分组失败：${error instanceof Error ? error.message : String(error)}`); } };
