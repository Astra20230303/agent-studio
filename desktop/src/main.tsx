...
import { clearThreadGoal, connectCodex, getThreadGoal, setThreadGoal, startUncommittedReview, subscribeCodex } from './codexClient';
...
function App({ initialState }: { initialState: DesktopState }) {
  ...
  const reviewActive = async () => {
    ...
    sendingRef.current.add(thread.id); setPendingThreads(previous => [...previous, thread.id]);
    try { const result = await startUncommittedReview(thread.remoteId); toast(`代码审查已开始（回合 ${result.turnId}）`); }
