...
export function GitPanel({ root, protectedPaths = [], onClose, onWorktree, onReview }: { root?: string; protectedPaths?: string[]; onClose: () => void; onWorktree: (project: import('./domain').Project) => void; onReview: (text: string) => void }) {
  ...
  if (history && root) return <section className="workspace-files git-panel" aria-label="Git 变更"><header><b>Git 提交历史</b><button onClick={() => setHistory(false)}>返回 Git 变更</button><button disabled={busy} aria-label="关闭 Git 面板" onClick={onClose}>×</button></header><GitHistory key={root} root={root} /></section>;
