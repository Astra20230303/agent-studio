...
export function GitPanel({ root, protectedPaths = [], onClose, onWorktree, onReview }: { root?: string; protectedPaths?: string[]; onClose: () => void; onWorktree: (project: import('./domain').Project) => void; onReview: (text: string) => void }) {
  ...
  if (worktrees && root) return <section className="workspace-files git-panel" aria-label="Git 变更"><header><b>Git 工作树</b><button disabled={busy} onClick={() => setWorktrees(false)}>返回 Git 变更</button><button disabled={busy} aria-label="关闭 Git 面板" onClick={onClose}>×</button></header><GitWorktrees protectedPaths={protectedPaths} key={root} root={root} onBusyChange={setBusy} onOpen={onWorktree} /></section>;
