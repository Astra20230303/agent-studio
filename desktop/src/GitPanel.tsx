...
export function GitPanel({ root, onClose, onWorktree, onReview }: { root?: string; onClose: () => void; onWorktree: (project: import('./domain').Project) => void; onReview: (text: string) => void }) {
  ...
  if (worktrees && root) return <section className="workspace-files git-panel" aria-label="Git 变更"><header><b>Git 工作树</b><button disabled={busy} onClick={() => setWorktrees(false)}>返回 Git 变更</button><button disabled={busy} aria-label="关闭 Git 面板" onClick={onClose}>×</button></header><GitWorktrees key={root} root={root} onBusyChange={setBusy} onOpen={onWorktree} /></section>;
