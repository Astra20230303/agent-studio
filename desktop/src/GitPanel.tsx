...
export function GitPanel({ root, onClose, onWorktree, onReview }: { root?: string; onClose: () => void; onWorktree: (project: import('./domain').Project) => void; onReview: (text: string) => void }) {
  const [snapshot, setSnapshot] = useState<{ branch: string; root: string; detached?: boolean; stashAvailable?: boolean; remotes?: string[]; upstream?: string; remote?: string; ahead?: number; behind?: number; files: ChangedFile[] }>();
