...
export function GitPanel({ root, onClose, onWorktree, onReview }: { root?: string; onClose: () => void; onWorktree: (project: import('./domain').Project) => void; onReview: (text: string) => void }) {
  const [snapshot, setSnapshot] = useState<{ branch: string; root: string; detached?: boolean; remotes?: string[]; upstream?: string; remote?: string; ahead?: number; behind?: number; files: ChangedFile[] }>();
  ...
  const mutate = async (action: string, path?: string) => {
    ...
    try {
      const result = await window.desktop?.workspaceGit?.({ root, action, path, message, branch, remote: publishRemote, expectedBranch: snapshot?.branch });
  ...
  if (branches && root) return <section className="workspace-files git-panel" aria-label="Git 变更"><header><b>Git 分支</b><button onClick={() => setBranches(false)}>返回 Git 变更</button><button aria-label="关闭 Git 面板" onClick={onClose}>×</button></header><GitBranches key={root} root={root} onSwitched={branch => { setBranches(false); setSelected(undefined); setSnapshot(undefined); setNotice(`已切换到 ${branch}`); setRevision(value => value + 1); }} /></section>;
