...
import { updateThreadGitInfo } from './codexClient';
export function GitPanel({ root, threadId, protectedPaths = [], onClose, onWorktree, onReview, onRecord }: { onRecord?: (action: string) => void; root?: string; threadId?: string; protectedPaths?: string[]; onClose: () => void; onWorktree: (project: import('./domain').Project) => void; onReview: (text: string) => void }) {
  ...
  const mutate = async (action: string, path?: string) => {
    ...
    try {
      ...
      recordGitAction(action, onRecord);
      if (action === 'create-worktree') { onWorktree(result.result); return; }
      if (action === 'pull' || action === 'pull-merge') setNotice(result.result.changed ? `已从 ${result.result.upstream} 更新到 ${result.result.commit.slice(0, 8)}` : '当前分支已是最新');
      ...
      if (action === 'stage-all') setNotice('已暂存全部变更');
      if (action === 'unstage-all') setNotice('已取消全部暂存，工作区内容保留');
    ...
    } catch (error) { setActionError(error instanceof Error ? error.message : String(error)); if (action === 'merge-branch' || action === 'pull-merge') { setSelected(undefined); setSnapshot(undefined); setRevision(value => value + 1); } }
    finally { lock.current = false; setBusy(false); }
  };
  const saveThreadMetadata = async () => {
    if (!threadId || !snapshot || metadataBusy) return;
    setMetadataBusy(true); setActionError(''); setNotice('');
    try { await updateThreadGitInfo(threadId, { sha: snapshot.head || null, branch: snapshot.branch || null }); setNotice('Git 元数据已保存到远端会话'); }
