...
export function duplicateTask(task: ScheduledTask, now = Date.now()): TaskDraft {
  const { name, prompt, kind, model, providerId, cwd, reasoningEffort, permission, notify, notificationPolicy } = task;
  ...
  return { name: `${name.slice(0, 115)}（副本）`, prompt, kind, model, providerId, cwd, reasoningEffort, permission, notify, notificationPolicy, schedule };
}
