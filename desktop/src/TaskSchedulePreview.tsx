...
export function TaskSchedulePreview({ schedule }: { schedule: TaskSchedule }) {
  ...
  const preview = async () => {
    ...
    try {
      ...
      if (active.current) setTimes(result.times);
  ...
  return <section aria-label="运行时间预览">
    ...
    {times && <><p>计划时间（本地时区：{localZone}）</p><ol>{times.map(time => <li key={time}><time dateTime={time}>{formatTaskDate(time)}</time></li>)}</ol><p>预览不创建任务。实际执行取决于应用运行状态和其他任务；固定间隔从保存或恢复时重新计时。</p></>}
  </section>;
}
