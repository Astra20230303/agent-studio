...
export function ServerWarnings({ warnings, dismiss }: Pick<ReturnType<typeof useServerWarnings>, 'warnings' | 'dismiss'>) {
  ...
  return <div className="state-save-warning">
    <span key={current.id} role="alert" style={{ whiteSpace: 'pre-wrap', overflowWrap: 'anywhere', minWidth: 0, maxHeight: '35vh', overflow: 'auto' }}>{current.message}{warnings.length > 1 && <small>（另有 {warnings.length - 1} 条待处理警告）</small>}</span>
