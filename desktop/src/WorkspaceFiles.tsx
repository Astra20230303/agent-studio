...
export function WorkspaceFiles({ root, onAttach, onClose, onEdit, previewUpdate }: { onEdit: (session: FileEditSession) => void; previewUpdate?: FilePreviewUpdate; root?: string; onAttach: (path: string) => void; onClose: () => void }) {
  ...
  return <section className="workspace-files" aria-label="工作区文件"><header><b>文件</b><button onClick={() => { setSelected(undefined); setDirectory('.'); setQuery(''); setSearch(''); }}>根目录</button><button onClick={() => setRefresh(value => value + 1)}>刷新文件</button><button onClick={onClose} aria-label="关闭文件面板">×</button></header><small>{root || '请先选择项目目录'}</small>
    ...
    {selected ? <><button onClick={() => setSelected(undefined)}>返回目录</button><h3>{selected.name}</h3><button onClick={() => onAttach(`${root}/${selected.path}`)}>添加到消息</button></> : <><p>{search ? `文件查找：${search}` : directory}</p>{!search && directory !== '.' && <button onClick={() => setDirectory(directory.split(/[\\/]/).slice(0, -1).join('/') || '.')}>上级目录</button>}{listing?.entries.map(entry => <button className="workspace-file-row" key={`${entry.path}:${entry.line || 0}`} onClick={() => entry.directory ? setDirectory(entry.path) : setSelected(entry)}>{entry.directory ? '▸' : '▧'} {search ? entry.path : entry.name}{entry.line ? `:${entry.line}:${entry.column} · ${entry.snippet}` : ''}{entry.symlink ? ' ↗' : ''}</button>)}{listing?.truncated && <p>{search ? '搜索达到上限，请缩小关键词范围。' : '仅显示前 1000 项。'}</p>}</>}
