...
export function WorkspaceFiles({ root, onAttach, onClose, onEdit, onPreview, previewUpdate }: { onPreview: (target: ArtifactTarget) => void; onEdit: (session: FileEditSession) => void; previewUpdate?: FilePreviewUpdate; root?: string; onAttach: (path: string) => void; onClose: () => void }) {
  ...
  useEffect(() => {
    ...
    if (!root) return;
    const read = async () => {
      const response = await window.desktop?.workspaceFile?.({ root, path: selected?.path || (search ? '.' : directory), action: selected ? 'read' : search ? contentSearch ? 'search-content' : 'search' : 'list', query: search, searchOptions: { caseSensitive, wholeWord } });
  ...
  }, [root, directory, selected, refresh, search, contentSearch, caseSensitive, wholeWord]);
