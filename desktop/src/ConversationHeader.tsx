...
export function ConversationHeader({ title, children, environment, actions }: { title: string; children: ReactNode; environment?: ReactNode; actions?: ReactNode }) {
  ...
  return <header className="conversation-header">
    ...
    <div className="conversation-header-actions">{actions && <ConversationPopover keepMounted label="更多会话操作" icon={<MoreHorizontal size={17} />}>{actions}</ConversationPopover>}{environment && <ConversationPopover keepMounted label="会话环境" icon={<Monitor size={17} />}>{environment}</ConversationPopover>}
