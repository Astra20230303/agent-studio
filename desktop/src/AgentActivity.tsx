...
export function AgentActivity({ tool, onOpenAgent }: { tool: ToolActivity; onOpenAgent?: (id: string) => void }) {
  if (tool.subAgent) {
    ...
    return <div className="tool-row" aria-label="子 Agent 活动" style={{ overflowWrap: 'anywhere' }}><p>{labels[activity.kind] || activity.kind}</p><p>{activity.path}</p><button disabled={!activity.threadId || !onOpenAgent} style={{ maxWidth: '100%', whiteSpace: 'normal', overflowWrap: 'anywhere' }} onClick={() => onOpenAgent?.(activity.threadId)}>打开 Agent {activity.threadId}</button><AgentControls key={activity.threadId} threadId={activity.threadId} /></div>;
  }
