...
(async () => {
  ...
  try {
    ...
    const mode=editor.getByRole('combobox',{name:'排队消息执行模式',exact:true});
    ...
    await model.selectOption('alternate');await effort.selectOption('high');await mode.selectOption('plan');
    await page.evaluate(()=>{
    ...
    assert.equal(await model.inputValue(),'alternate');assert.equal(await effort.inputValue(),'high');assert.equal(await mode.inputValue(),'plan');
    assert.equal(await page.evaluate(()=>JSON.parse(localStorage.getItem('felix-turn-queue-v1'))[0].planningMode),'default');
    assert.equal(await page.evaluate(()=>JSON.parse(localStorage.getItem('felix-turn-queue-v1'))[0].model),'test');
    ...
    assert.equal(saved.planningMode,'plan');
    await page.getByLabel('排队执行配置',{exact:true}).filter({hasText:'alternate · 高 · 先规划'}).waitFor();
    ...
    assert.equal(await page.evaluate(()=>JSON.parse(localStorage.getItem('felix-turn-queue-v1'))[0].planningMode),'default');
    await page.getByLabel('排队执行配置',{exact:true}).filter({hasText:'alternate · 高 · 直接执行'}).waitFor();
    await open.click();await mode.selectOption('plan');
    ...
    assert.equal(after.planningMode,before.planningMode);assert.equal(after.model,before.model);assert.equal(after.reasoningEffort,before.reasoningEffort);
    ...
    assert.equal(sent.collaborationMode.mode,'plan');
