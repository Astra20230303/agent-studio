...
(async () => {
  ...
  try {
    ...
    await page.getByRole('button', { name: '关闭终端查找', exact: true }).click();
    await page.getByRole('button', { name: '重命名当前终端', exact: true }).click();
    assert.equal(await terminalName.getAttribute('maxlength'), '80');
    const longName = '终端构建任务'.repeat(12);
    await terminalName.fill(longName);
    await terminalName.dispatchEvent('keydown', { key: 'Enter', code: 'Enter', isComposing: true, bubbles: true });
    ...
    assert.ok(tabBox.width <= 180);
    assert.equal(await page.getByRole('button', { name: '重命名当前终端', exact: true }).isVisible(), true);
    assert.deepEqual([...manager.sessions.keys()], ids);
    ...
    assert.equal(await longTab.count(), 1, 'Restart preserves terminal tab name');
    await page.getByRole('button', { name: '关闭当前终端', exact: true }).click();
