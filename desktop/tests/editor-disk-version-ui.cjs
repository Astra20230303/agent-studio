...
(async () => {
  ...
  try {
    ...
    await inspect.click();
    await page.getByLabel('磁盘快照内容', { exact: true }).filter({ hasText: '（空文件）' }).waitFor();
    assert.equal(await editor.inputValue(), 'draft\n中文');
    await page.setViewportSize({ width: 600, height: 800 });
    assert.equal(await page.locator('.editor-version-columns').evaluate(node => getComputedStyle(node).gridTemplateColumns.split(' ').length), 1);
    await page.setViewportSize({ width: 1280, height: 900 });
    assert.equal(await page.locator('.editor-version-columns').evaluate(node => getComputedStyle(node).gridTemplateColumns.split(' ').length), 2);
