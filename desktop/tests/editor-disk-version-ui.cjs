...
(async () => {
  ...
  try {
    ...
    assert.equal(await compare.count(), 0);
    assert.equal(await editor.inputValue(), 'draft\n中文');
    await fs.writeFile(target, 'external one\r\n中文');
    await inspect.click();
    const adopt = page.getByRole('button', { name: '保留草稿，采用此磁盘版本作为基准', exact: true });
    await adopt.waitFor();
    page.once('dialog', dialog => dialog.dismiss());
    await adopt.click();
    assert.equal(await page.getByLabel('编辑起点内容', { exact: true }).textContent(), 'original\r\n中文');
    page.once('dialog', dialog => {
      assert.match(dialog.message(), /不会自动合并或立即写入/);
      return dialog.accept();
    });
    await adopt.click();
    assert.equal(await adopt.isDisabled(), true);
    assert.equal(await editor.inputValue(), 'draft\n中文');
    assert.equal(await fs.readFile(target, 'utf8'), 'external one\r\n中文');
    assert.equal(writes.length, 1);
    await editor.press('Control+z');
    assert.equal(await editor.inputValue(), 'original\n中文');
    await editor.press('Control+y');
    assert.equal(await editor.inputValue(), 'draft\n中文');
    await fs.writeFile(target, 'external two\r\n中文');
    await page.getByRole('button', { name: '保存文件', exact: true }).click();
    await page.getByRole('alert').filter({ hasText: '外部修改' }).waitFor();
    ...
    await inspect.click();
    await adopt.waitFor();
    await editor.fill('external two\n中文\nmy addition');
