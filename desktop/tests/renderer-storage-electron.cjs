...
(async () => {
  ...
  let app;
  const launch = async (waitForEditor = true) => {
    ...
    if (waitForEditor) await page.getByRole('textbox', { name: '消息', exact: true }).waitFor();
    return page;
  };
  ...
  try {
    ...
    await page.getByRole('textbox', { name: '消息', exact: true }).fill('Retry native write');
    const retrySave = page.getByRole('button', { name: '重试保存草稿', exact: true });
    await retrySave.waitFor();
    assert.equal(await page.getByRole('textbox', { name: '消息', exact: true }).inputValue(), 'Retry native write');
    fs.rmdirSync(file);
    await retrySave.click(); await retrySave.waitFor({ state: 'detached' });
    assert.equal(JSON.parse(JSON.parse(fs.readFileSync(file, 'utf8')).value).legacy, 'Retry native write');
    await app.close(); app = undefined;
    ...
    fs.writeFileSync(file, '{broken primary'); fs.writeFileSync(file + '.bak', '{broken backup');
    page = await launch(false);
    const retryRead = page.getByRole('button', { name: '重试读取', exact: true });
    await retryRead.waitFor();
    assert.equal(await page.getByRole('textbox', { name: '消息', exact: true }).count(), 0);
    assert.equal(fs.readFileSync(file, 'utf8'), '{broken primary');
    assert.equal(fs.readFileSync(file + '.bak', 'utf8'), '{broken backup');
    fs.writeFileSync(file, repair);
    await retryRead.click();
    await page.getByRole('textbox', { name: '消息', exact: true }).waitFor();
    assert.equal(await page.getByRole('textbox', { name: '消息', exact: true }).inputValue(), backup);
    console.log('PASS: real Electron migration, cleared browser storage, native save retry, backup recovery and corrupt-store startup protection');
  } finally {
