...
(async()=>{
 ...
 try {
  ...
  assert.equal(await page.evaluate(()=>JSON.parse(localStorage.getItem('felix-audit-log-v1')||'[]').filter(entry=>entry.action.startsWith('Git ')).length),0);
  ...
  assert.deepEqual(audit.map(entry=>entry.action),['Git 提交','Git 暂存全部变更','Git 取消全部暂存','Git 暂存全部变更']);
  assert.ok(audit.every(entry=>entry.detail===undefined));
  assert.ok(!JSON.stringify(audit).includes(root));assert.ok(!JSON.stringify(audit).includes('bulk change'));
