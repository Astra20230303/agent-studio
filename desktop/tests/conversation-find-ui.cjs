...
(async () => {
  ...
  try {
    ...
    await page.addInitScript(() => {
      ...
      window.codex = { connect: async () => ({ ok: true }), notify: async () => ({}), request: async method => ({ ok: true, result: method === 'thread/resume' ? { thread: { turns: [] } } : { data: [] } }), onNotification: fn => { const listeners=window.__listeners ||= []; listeners.push(fn); window.__notify=message=>listeners.forEach(listener=>listener(message)); return () => { const index=listeners.indexOf(fn); if(index>=0)listeners.splice(index,1); }; }, onServerRequest: () => () => {}, onClosed: () => () => {}, onError: () => () => {}, onStderr: () => () => {} };
    });
    ...
    assert.equal(await page.getByRole('searchbox', { name: '查找会话内容' }).count(), 0);
    assert.ok(await rename.evaluate(element => element.contains(document.activeElement)));
    ...
    await page.evaluate(() => { window.__notify({method:'turn/started',params:{threadId:'remote-a',turn:{id:'search-turn'}}}); window.__notify({ method: 'item/agentMessage/delta', params: { threadId: 'remote-a', turnId:'search-turn', itemId: 'streaming', delta: 'Incoming reply during search' } }); });
