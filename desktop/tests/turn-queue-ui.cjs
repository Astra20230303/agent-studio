...
(async () => {
  ...
  try {
    ...
    assert.ok(await page.getByRole('button', { name: '上移排队消息：edited first', exact: true }).isDisabled());
    assert.ok(await page.getByRole('button', { name: '下移排队消息：second', exact: true }).isDisabled());
    await page.evaluate(() => { window.__queueQuota = true; });
    await page.getByRole('button', { name: '上移排队消息：second', exact: true }).click();
    await page.getByRole('button', { name: '重试保存队列', exact: true }).waitFor();
    assert.deepEqual(await page.evaluate(() => JSON.parse(localStorage.getItem('felix-turn-queue-v1')).map(item => item.text)), ['edited first', 'second']);
    await page.evaluate(() => { window.__queueQuota = false; });
    await page.getByRole('button', { name: '上移排队消息：second', exact: true }).click();
    assert.deepEqual(await page.evaluate(() => JSON.parse(localStorage.getItem('felix-turn-queue-v1')).map(item => item.text)), ['second', 'edited first']);
    await finish('completed');
    await page.getByRole('button', { name: '继续队列', exact: true }).waitFor();
    assert.equal(await page.evaluate(() => window.__sent.length), 0);
    await page.getByRole('button', { name: '继续队列', exact: true }).click();
    await page.waitForFunction(() => window.__sent.length === 1);
    assert.equal(await page.evaluate(() => window.__sent[0].input[0].text), 'second');
    ...
    assert.equal(await page.evaluate(() => window.__sent[2].input[0].text), 'edited first');
    assert.deepEqual(await page.evaluate(() => window.__sent[2].input.filter(item => item.type === 'localImage')), [{ type: 'localImage', path: 'D:/keep.png' }]);
