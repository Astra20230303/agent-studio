...
(async () => {
  ...
  try {
    ...
    await page.evaluate(() => window.__listeners.forEach(fn => fn({ method: 'turn/started', params: { threadId: 'a', turn: { id: 'running-turn', status: 'inProgress' } } })));
    await page.getByRole('button', { name: '停止生成', exact: true }).waitFor();
    assert.ok(await panel.getByRole('button', { name: '启动', exact: true }).isDisabled());
