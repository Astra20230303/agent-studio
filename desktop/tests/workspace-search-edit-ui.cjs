...
(async () => {
 ...
 try {
  ...
  await fs.writeFile(file, 'needle + 中文🙂 + needle\r\n');
  await panel.getByRole('button', { name: '返回目录', exact: true }).click();
  await query.fill('needle'); await query.press('Enter');
  ...
  assert.deepEqual(await editor.evaluate(node => [node.selectionStart, node.value.slice(node.selectionStart, node.selectionEnd)]), ['needle + 中文🙂 + '.length, 'needle']);
  await page.keyboard.insertText('second only');
  await editor.press('Control+s'); await editor.waitFor({ state: 'detached' });
  assert.equal(await fs.readFile(file, 'utf8'), 'needle + 中文🙂 + second only\r\n');
  assert.equal(writes.length, 3);
  console.log('PASS: real search/edit/save, repeated Unicode-offset matches via preview, CRLF and external change protection');
