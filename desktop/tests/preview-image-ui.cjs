...
(async () => {
  ...
  try {
    ...
    const fittedWidth = (await img.boundingBox()).width;
    await region.getByRole('button', { name: '缩小图片', exact: true }).click();
    assert.ok(Math.abs((await img.boundingBox()).width - fittedWidth / 1.25) < 1);
    for (let i = 0; i < 30 && !await region.getByRole('button', { name: '缩小图片', exact: true }).isDisabled(); i++) await region.getByRole('button', { name: '缩小图片', exact: true }).click();
    assert.ok(await region.getByRole('button', { name: '缩小图片', exact: true }).isDisabled());
    assert.equal((await img.boundingBox()).width, 160);
    for (let i = 0; i < 30 && !await region.getByRole('button', { name: '放大图片', exact: true }).isDisabled(); i++) await region.getByRole('button', { name: '放大图片', exact: true }).click();
    assert.ok(await region.getByRole('button', { name: '放大图片', exact: true }).isDisabled());
    assert.equal((await img.boundingBox()).width, 12800);
    await region.getByRole('button', { name: '适应窗口', exact: true }).click();
    await page.evaluate(() => { document.body.firstElementChild.style.width = '50px'; });
    await page.waitForFunction(() => document.querySelector('.preview-image img').getBoundingClientRect().width <= 50);
    await region.getByRole('button', { name: '放大图片', exact: true }).click();
    assert.equal((await img.boundingBox()).width, 160);
    await page.evaluate(() => { document.body.firstElementChild.style.width = '500px'; });
    await region.getByRole('button', { name: '放大图片', exact: true }).click();
