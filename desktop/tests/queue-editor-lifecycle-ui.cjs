...
(async () => {
  ...
  try {
    ...
    await editor.getByRole('textbox').fill('keep typing through refresh');
    ...
    assert.deepEqual(await page.evaluate(() => window.__saved), [['b', 'keep typing through refresh', [], { model: 'test', effort: 'low' }]]);
    await page.getByRole('button', { name: '编辑排队消息：B', exact: true }).click();
    await page.evaluate(() => window.__renderQueue([]));
    await editor.waitFor({ state: 'detached' });
    await page.getByRole('region', { name: '待发送消息', exact: true }).waitFor({ state: 'detached' });
    await page.evaluate(() => window.__renderQueue([window.__b]));
