...
(async()=>{const browser=await chromium.launch({channel:'msedge',headless:true});try{
 const page=await browser.newPage();await page.addInitScript(()=>{
  const content=['#### Details','','> quoted **strong**','','- parent','  - nested *emphasis*','- [x] completed','- [ ] pending','','3. third','4. fourth','','---','','[reference][docs]','','[docs]: https://example.com/docs','','`` &amp; `code` `` and &amp;','','<script>window.__executed=true</script>','','```js','const x = "<tag>";','```'].join('\n');
