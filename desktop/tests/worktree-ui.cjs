...
(async () => { const browser = await chromium.launch({ channel: 'msedge', headless: true }); try {
  const page = await browser.newPage(); await page.addInitScript(() => {
    ...
    window.__requests = [];
    window.desktop = { workspaceGit: async input => { window.__requests.push(input); return { ok: true, result: input.action === 'status' ? { root: 'D:/repo', branch: 'main', files: [] } : { id: 'D:/worktree', path: 'D:/worktree', name: 'Worktree', environment: 'worktree', git: { isRepository: true, branch: input.branch } } }; }, listModels: async () => ({ ok: true, models: ['test'] }) };
  ...
  assert.equal(audit.find(item => item.action === '切换项目').detail, undefined);
