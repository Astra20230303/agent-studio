...
test('run snapshot survives edits, failure and restart without copying credentials or histories',async()=>{
 ...
 let scheduler=new TaskScheduler({directory,runner:async()=>{if(fail)throw Error('fixture');return {output:'ok'};}});
 try{
  let task=scheduler.save({name:'Before',prompt:'Original prompt',kind:'agent',model:'original-model',providerId:'original',permission:'read-only',reasoningEffort:'high',notify:true,schedule:{kind:'interval',minutes:60},apiKey:'never-copy'});
  ...
  const runs=scheduler.detail(task.id).runs;assert.equal(runs[0].status,'failed');assert.equal(runs[0].configuration.model,'new-model');assert.equal(runs[1].configuration.model,'original-model');assert.equal(runs[1].configuration.reasoningEffort,'high');assert.equal(runs[1].configuration.name,'Before');
