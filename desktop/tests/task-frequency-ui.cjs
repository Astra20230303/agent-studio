...
(async()=>{const browser=await chromium.launch({channel:'msedge',headless:true});try{
 ...
 await editor.getByLabel('运行时间',{exact:true}).fill('18:45');await editor.getByRole('combobox',{name:'任务时区',exact:true}).selectOption('America/New_York');
 ...
 await editor.getByRole('button',{name:'预览运行时间',exact:true}).click();await editor.locator('time').waitFor();
 assert.deepEqual(await page.evaluate(()=>window.__previews.at(-1)),{kind:'monthly',time:'18:45',timezone:'America/New_York',monthDay:31});
 await frequency.selectOption('daily');assert.equal(await editor.locator('time').count(),0);
 assert.equal(await editor.getByLabel('运行时间',{exact:true}).inputValue(),'18:45');assert.equal(await editor.getByRole('combobox',{name:'任务时区',exact:true}).inputValue(),'America/New_York');
 await editor.getByRole('button',{name:'保存任务',exact:true}).click();await editor.waitFor({state:'detached'});
 assert.deepEqual(await page.evaluate(()=>window.__saves[0].schedule),{kind:'daily',time:'18:45',timezone:'America/New_York'});
