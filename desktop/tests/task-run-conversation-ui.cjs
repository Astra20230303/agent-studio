...
(async()=>{const browser=await chromium.launch({channel:'msedge',headless:true});try{
 const page=await browser.newPage();await page.addInitScript(()=>{
  localStorage.setItem('codex-desktop-state-v1',JSON.stringify({activeThreadId:'local',threads:[{id:'local',title:'Original chat',status:'completed',messages:[],updatedAt:''}]}));
  ...
  window.codex={connect:async()=>({ok:true}),notify:async()=>({}),request:async(method,params)=>{if(method==='thread/resume'){window.__resumes.push(params.threadId);return {ok:true,result:{thread:{id:params.threadId,turns:[{id:'turn',items:[{id:'answer',type:'agentMessage',text:'Restored task conversation'}]}]}}};}return {ok:true,result:{data:[]}};},onNotification:()=>()=>{},onServerRequest:()=>()=>{},onClosed:()=>()=>{},onError:()=>()=>{},onStderr:()=>()=>{}};
 ...
 await page.getByRole('button',{name:'Original chat',exact:true}).click();assert.equal(await page.getByRole('textbox',{name:'消息',exact:true}).inputValue(),'Original draft');
