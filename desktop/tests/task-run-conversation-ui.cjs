...
(async()=>{const browser=await chromium.launch({channel:'msedge',headless:true});try{
 const page=await browser.newPage();await page.addInitScript(()=>{
  ...
  window.codex={connect:async()=>({ok:true}),notify:async()=>({}),request:async(method,params)=>{if(method==='thread/resume'){window.__resumes.push(params.threadId);if(window.__failResume)return {ok:false,error:'Task resume unavailable'};return {ok:true,result:{thread:{id:params.threadId,turns:[{id:'turn',items:[{id:'answer',type:'agentMessage',text:'Restored task conversation'}]}]}}};}return {ok:true,result:{data:[]}};},onNotification:()=>()=>{},onServerRequest:()=>()=>{},onClosed:()=>()=>{},onError:()=>()=>{},onStderr:()=>()=>{}};
