...
(async () => { const browser = await chromium.launch({ channel: 'msedge', headless: true }); try {
  const page = await browser.newPage(); await page.addInitScript(() => {
    ...
    window.__requests = [];
    window.desktop = { workspaceGit: async input => { window.__requests.push(input); if(input.action==='worktrees')return {ok:true,result:{worktrees:[{path:'D:/existing',branch:'codex/existing'}]}}; return { ok: true, result: input.action === 'status' ? { root: 'D:/repo', branch: 'main', files: [] } : { id: 'wt', path: 'D:/worktree', name: 'Worktree', environment: 'worktree', git: { isRepository: true, branch: input.branch } } }; }, listModels: async () => ({ ok: true, models: ['test'] }) };
