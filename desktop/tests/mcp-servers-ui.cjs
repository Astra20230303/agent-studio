  ...
  try {
    ...
    await page.addInitScript(() => {
      ...
      window.codex = { connect: async () => ({ ok: true }), notify: async () => ({}), request: async (method, params) => {
        ...
        if (method === 'mcpServerStatus/list' && params.detail === 'full') return { ok: true, result: { data: [{ name: 'local', authStatus: 'unsupported', resources: [{ uri: 'fixture://readme', name: 'Readme', description: 'Workspace guide' }], resourceTemplates: [{ uriTemplate: 'fixture://notes/{id}', name: 'Notes' }, { uriTemplate: 'fixture://search{?q,page}', name: 'Search' }, { uriTemplate: 'fixture://broken/{id', name: 'Broken' }, { uriTemplate: 'fixture://collection{/parts*}{?filters*}', name: 'Collection' }] }] } };
    ...
    await page.getByText('资源模板 (4)', { exact: true }).click();
    ...
    await page.getByRole('textbox', { name: 'Search 参数 q', exact: true }).fill('a&b=42');
    assert.equal(await page.getByLabel('Search 生成的 URI', { exact: true }).textContent(), 'fixture://search?q=a%26b%3D42');
    await page.getByRole('button', { name: '读取 Search 模板资源', exact: true }).click();
    await page.getByLabel('资源内容').getByText('fixture://search?q=a%26b%3D42', { exact: true }).waitFor();
    await page.getByText('Broken', { exact: true }).click();
    await page.getByRole('alert').filter({ hasText: '资源模板格式无效' }).waitFor();
    assert.equal(await page.getByRole('button', { name: '读取 Broken 模板资源', exact: true }).count(), 0);
    await page.getByText('Broken', { exact: true }).click();
    await page.getByText('Collection', { exact: true }).click();
    const parts = page.getByRole('textbox', { name: 'Collection 参数 parts', exact: true });
    const filters = page.getByRole('textbox', { name: 'Collection 参数 filters', exact: true });
    const collectionRead = page.getByRole('button', { name: '读取 Collection 模板资源', exact: true });
    await page.getByRole('combobox', { name: 'Collection 参数 parts 类型', exact: true }).selectOption('array');
    await parts.fill('["a/b","中文"]');
    await page.getByRole('combobox', { name: 'Collection 参数 filters 类型', exact: true }).selectOption('object');
    await filters.fill('{"q":"a&b","page":"2"}');
    const collectionUri = 'fixture://collection/a%2Fb/%E4%B8%AD%E6%96%87?q=a%26b&page=2';
    assert.equal(await page.getByLabel('Collection 生成的 URI', { exact: true }).textContent(), collectionUri);
    const beforeInvalid = await page.evaluate(() => window.__calls.filter(call => call.method === 'mcpServer/resource/read').length);
    await parts.fill('[1]');
    await page.getByRole('alert').filter({ hasText: 'parts: 请输入 JSON 字符串数组' }).waitFor();
    assert.equal(await collectionRead.isDisabled(), true);
    assert.equal(await page.getByLabel('Collection 生成的 URI', { exact: true }).count(), 0);
    assert.equal(await parts.isEnabled(), true, 'Invalid input must remain editable');
    await parts.press('Enter');
    assert.equal(await page.evaluate(() => window.__calls.filter(call => call.method === 'mcpServer/resource/read').length), beforeInvalid);
    await parts.fill('["a/b","中文"]');
    await collectionRead.click();
    await page.getByLabel('资源内容').getByText(collectionUri, { exact: true }).waitFor();
    assert.equal(await page.evaluate(() => window.__calls.filter(call => call.method === 'mcpServer/resource/read').at(-1).params.uri), collectionUri);
    await filters.fill('{"q":{"nested":"invalid"}}');
    await page.getByRole('alert').filter({ hasText: 'filters: 请输入值均为字符串的 JSON 对象' }).waitFor();
    assert.equal(await collectionRead.isDisabled(), true);
    await filters.fill('{}');
    await parts.fill('[]');
    assert.equal(await page.getByLabel('Collection 生成的 URI', { exact: true }).textContent(), 'fixture://collection');
    await collectionRead.click();
    await page.getByLabel('资源内容').getByText('fixture://collection', { exact: true }).waitFor();
    const partsKind = page.getByRole('combobox', { name: 'Collection 参数 parts 类型', exact: true });
    await parts.fill('["preserved"]');
    await partsKind.selectOption('text');
    assert.equal(await parts.inputValue(), '["preserved"]');
    assert.equal(await page.getByLabel('Collection 生成的 URI', { exact: true }).textContent(), 'fixture://collection/%5B%22preserved%22%5D');
    await partsKind.selectOption('array');
    assert.equal(await page.getByLabel('Collection 生成的 URI', { exact: true }).textContent(), 'fixture://collection/preserved');
    console.log('PASS: composite template values, invalid input isolation and recovery');
    const resourceSearch = page.getByRole('searchbox', { name: 'local 搜索资源和模板', exact: true });
    const resourceReadsBeforeSearch = await page.evaluate(() => window.__calls.filter(call => call.method === 'mcpServer/resource/read').length);
    await resourceSearch.fill('  GUIDE  ');
    await page.getByRole('button', { name: 'Readme', exact: true }).waitFor();
    assert.equal(await page.getByRole('textbox', { name: 'Collection 参数 parts', exact: true }).count(), 0);
    await resourceSearch.fill('fixture://collection');
    assert.equal(await page.getByRole('button', { name: 'Readme', exact: true }).count(), 0);
    assert.equal(await parts.inputValue(), '["preserved"]', 'Filtering preserves template edits');
    await resourceSearch.fill('notes');
    assert.equal(await page.getByRole('textbox', { name: 'Notes 参数 id', exact: true }).inputValue(), 'a/b 中文');
    await resourceSearch.fill('missing-resource');
    await page.getByText('没有匹配的资源或模板，仍可手动输入 URI 读取。', { exact: true }).waitFor();
    await page.getByLabel('资源内容').getByText('fixture://collection', { exact: true }).waitFor();
    assert.equal(await page.getByRole('textbox', { name: 'local 资源 URI', exact: true }).isEnabled(), true);
    await page.getByRole('button', { name: '清空 local 资源搜索', exact: true }).click();
    await page.getByRole('button', { name: 'Readme', exact: true }).waitFor();
    assert.equal(await parts.inputValue(), '["preserved"]');
    assert.equal(await page.evaluate(() => window.__calls.filter(call => call.method === 'mcpServer/resource/read').length), resourceReadsBeforeSearch);
    console.log('PASS: resource search by description/name/URI, no-results recovery and template/content preservation');
    ...
    await readUri('fixture://cancel/search-in-flight');
    await page.waitForFunction(() => !!window.__pendingResources?.['fixture://cancel/search-in-flight']);
    await resourceSearch.fill('no-match-during-read');
    await page.getByRole('status').filter({ hasText: '正在读取资源' }).waitFor();
    assert.equal(await page.getByRole('button', { name: '取消等待读取', exact: true }).isEnabled(), true);
    await page.evaluate(() => window.__pendingResources['fixture://cancel/search-in-flight'].resolve({ ok: true, result: { contents: [{ uri: 'fixture://search-result', text: 'Search did not cancel this read' }] } }));
    await page.getByLabel('资源内容').getByText('fixture://search-result', { exact: true }).waitFor();
    assert.equal(await page.getByRole('button', { name: '加入聊天草稿', exact: true }).isEnabled(), true);
    await page.getByRole('button', { name: '清空 local 资源搜索', exact: true }).click();
    assert.equal(await parts.inputValue(), '["preserved"]');
    assert.equal(await page.getByRole('combobox', { name: 'Collection 参数 parts 类型', exact: true }).inputValue(), 'array');
    await page.getByText('资源模板 (4)', { exact: true }).click();
    assert.equal(await parts.count(), 0, 'Template directory can still be collapsed manually');
    await resourceSearch.fill('collection');
    await parts.waitFor();
    assert.equal(await parts.inputValue(), '["preserved"]');
    await page.getByRole('button', { name: '清空 local 资源搜索', exact: true }).click();
    console.log('PASS: search preserves in-flight read, returned snapshot, parameter types and manual directory controls');
    for (const outcome of ['success', 'failure']) {
      ...
      assert.equal(await page.getByRole('textbox', { name: 'Notes 参数 id', exact: true }).isDisabled(), true);
      assert.equal(await page.getByRole('button', { name: '读取 Notes 模板资源', exact: true }).isDisabled(), true);
      await page.getByRole('button', { name: '取消等待读取', exact: true }).click();
      await page.getByRole('status').filter({ hasText: '已取消等待' }).waitFor();
      assert.equal(await page.getByRole('textbox', { name: 'Notes 参数 id', exact: true }).isEnabled(), true);
      assert.equal(await page.getByRole('textbox', { name: 'Notes 参数 id', exact: true }).inputValue(), 'a/b 中文');
