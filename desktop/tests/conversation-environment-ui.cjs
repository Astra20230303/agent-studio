...
await page.addInitScript(scenario=>{
  ...
  window.desktop={workspaceGit:async input=>{window.__gitCalls=(window.__gitCalls||0)+1;if(window.__gitFail)return {ok:false,error:'Git unavailable'};return {ok:true,result:{root:input.root,branch:'feature/test',files:[{path:'sample.ts',index:' ',working:'M',untracked:false}]}};},listModels:async()=>({ok:true,models:['test']}),providerStatus:async()=>({keyConfigured:true})};
