...
test('real app-server terminates a retained background command after interruption and continues the same conversation', { timeout: 30000 }, async () => {
  ...
  try {
    ...
    const terminals = await rpc.request('thread/backgroundTerminals/list', { threadId: thread.id });
    ...
    const terminated = await rpc.request('thread/backgroundTerminals/terminate', { threadId: thread.id, processId: terminal.processId });
