...
(async()=>{const browser=await chromium.launch({channel:'msedge',headless:true});try{
 const page=await browser.newPage();await page.addInitScript(()=>{
  window.__task={id:'a',name:'Metadata',prompt:'Inspect settings',kind:'agent',providerId:'known',model:'test',reasoningEffort:'high',permission:'read-only',notify:true,notificationPolicy:'failed_runs_only',status:'active',nextRunAt:null,schedule:{kind:'daily',time:'09:00',timezone:'UTC'},runs:[{id:'r',status:'completed',trigger:'manual',startedAt:'2026-09-18T00:00:00Z',configuration:{name:'Historical name',prompt:'Historical prompt',kind:'agent',model:'historical-model',permission:'read-only',reasoningEffort:'low'}}]};
 ...
 await page.getByText('historical-model',{exact:true}).waitFor();await page.getByText('Historical prompt',{exact:true}).waitFor();
