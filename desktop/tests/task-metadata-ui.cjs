...
(async()=>{const browser=await chromium.launch({channel:'msedge',headless:true});try{
 const page=await browser.newPage();await page.addInitScript(()=>{
  ...
  window.__task={id:'a',name:'Metadata',prompt:'Inspect settings',kind:'agent',providerId:'known',model:'test',reasoningEffort:'high',permission:'read-only',notify:true,notificationPolicy:'failed_runs_only',status:'active',nextRunAt:null,schedule:{kind:'daily',time:'09:00',timezone:'UTC'},runs:[{outputTruncated:true,id:'r',status:'completed',trigger:'manual',startedAt:'2026-09-18T00:00:00Z',environment:{cwd:'D:/resolved-workspace',providerId:'resolved-provider'},configuration:{name:'Historical name',prompt:'Historical prompt',kind:'agent',model:'historical-model',permission:'read-only',reasoningEffort:'low'}}]};
 ...
 await page.locator('.task-run > summary').click();await page.getByText('输出过长，仅保留末尾 200000 字符。',{exact:true}).waitFor();
 ...
 const exported=await page.evaluate(()=>window.__copied);assert.ok(exported.startsWith('Historical name\n'));assert.ok(exported.includes('输出过长，仅保留末尾 200000 字符。'));assert.match(exported,/实际执行环境[\s\S]*D:\/resolved-workspace[\s\S]*resolved-provider/);
