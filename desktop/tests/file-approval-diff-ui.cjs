...
(async()=>{const browser=await chromium.launch({channel:'msedge',headless:true});try{
 ...
 window.__responses=[];const listeners=new Set();window.__notify=message=>listeners.forEach(fn=>fn(message));window.desktop={listModels:async()=>({ok:true,models:['test']})};
 window.codex={connect:async()=>({ok:true}),request:async method=>({ok:true,result:method==='thread/resume'?{thread:{turns:[]}}:{data:[]}}),notify:async()=>({}),respond:async(id,result)=>{window.__responses.push({id,result});return{ok:true}},onNotification:fn=>{listeners.add(fn);return()=>listeners.delete(fn)},onServerRequest:fn=>{window.__ask=fn;return()=>{}},onClosed:()=>()=>{},onError:()=>()=>{},onStderr:()=>()=>{}};
 ...
 await page.evaluate(()=>window.__notify({method:'item/started',params:{threadId:'remote',turnId:'turn',item:{id:'file',type:'fileChange',changes:[]}}}));
