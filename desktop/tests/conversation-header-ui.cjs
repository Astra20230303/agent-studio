...
await page.addInitScript(scenario=>{
 ...
 await page.locator('.conversation-header > .conversation-settings summary').click();
 await page.getByRole('textbox',{name:'会话目标',exact:true}).fill('Keep my goal draft');
 await page.keyboard.press('Escape');await page.waitForFunction(()=>!document.querySelector('.conversation-header > .conversation-settings').matches(':popover-open'));
 ...
 await page.waitForFunction(()=>!document.querySelector('.conversation-header > .conversation-settings').matches(':popover-open'));
 await settings.click();
 ...
 const box=await page.locator('.conversation-header > .conversation-settings').boundingBox();assert.ok(box.x>=0&&box.x+box.width<=600&&box.y+box.height<=650);
 await page.screenshot({path:'.project-cache/ui-checks/conversation-settings-600.png'});
 await page.setViewportSize({width:390,height:600});
 const narrow=await page.locator('.conversation-header > .conversation-settings').boundingBox();assert.ok(narrow.x>=0&&narrow.x+narrow.width<=390&&narrow.y+narrow.height<=600);
 ...
 const more=page.getByRole('button',{name:'更多会话操作',exact:true});await more.click();
 await page.getByRole('button',{name:'导出 Markdown',exact:true}).waitFor();
 ...
 assert.equal(await page.locator('.conversation-header > .conversation-settings').evaluate(el=>el.matches(':popover-open')),false);
 await settings.click();
 await page.locator('.conversation-header > .conversation-settings summary').click();
