...
(async () => {
  ...
  try {
    ...
    await editor.fill('Temporarily unsaved');
    await retry.waitFor();
    await page.evaluate(() => { window.__failDraft = false; });
    await editor.fill('Automatically saved after recovery');
    await retry.waitFor({ state: 'detached' });
    assert.equal(await page.evaluate(() => JSON.parse(localStorage.getItem('felix-thread-drafts-v1')).new), 'Automatically saved after recovery');
    await page.evaluate(() => { window.__failDraft = true; });
