...
(async () => {
 ...
 try {
  ...
  await page.getByRole('button',{name:'会话环境',exact:true}).click();
  ...
  await page.keyboard.press('Escape');await page.getByRole('button',{name:'会话环境',exact:true}).click();assert.ok(await kill.isDisabled());
