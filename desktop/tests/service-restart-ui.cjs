...
(async()=>{const browser=await chromium.launch({channel:'msedge',headless:true});try{
 const page=await browser.newPage();await page.addInitScript(()=>{
  ...
  window.codex={connect:async()=>{window.__connections++;return {ok:true};},stop:async()=>{window.__stops++;if(window.__stopFail)return {ok:false,error:'Stop failed'};closed.forEach(fn=>fn({}));await new Promise(resolve=>{window.__releaseStop=resolve;});return {ok:true};},notify:async()=>({}),request:async method=>({ok:true,result:method==='thread/resume'?{thread:{id:'remote',turns:[]}}:{data:[]}}),onNotification:fn=>{notifications.add(fn);return()=>notifications.delete(fn);},onClosed:fn=>{closed.add(fn);return()=>closed.delete(fn);},onError:()=>()=>{},onStderr:()=>()=>{},onServerRequest:()=>()=>{}};
 });
