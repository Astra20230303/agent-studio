...
- 提交后 Edge 界面验收：星期五转多日保持星期五、增加星期二后转单日为星期二，月度再转每天后保存请求不含旧 day/days/monthDay，时间/纽约时区保持，每次切换清除旧预览。补齐时区下拉明确 aria-label“任务时区”，避免名称混入选项文本；界面专项与生产构建通过。

### 完整任务时区目录（2026-09-19）

- 已交付：任务时区下拉由固定短名单扩展为 Intl.supportedValuesOf 提供的运行环境时区目录，排序去重，同时保留 UTC、本地时区和已有任务别名；目录 API 不可用时使用兼容短名单且保留当前值。
- 目录测试覆盖巴黎/悉尼/奥克兰、别名保留、去重和 API 失败。Edge 验证悉尼选择跨频率切换、预览与保存请求保持准确；7 项调度预览测试包含南半球夏令时切换并通过，生产构建通过。具体地区目录由运行时 ICU 数据决定，最终安排仍由原生调度器验证。

- 提交后真实 Electron 验收：目录选择 Australia/Sydney，原生预览的三次时间换算到悉尼均为每月 15 日 09:00；明确保存并重启后详情、编辑选项和正式存储仍保持悉尼时区。完整原生任务草稿/保存/复制/删除链路通过，无产品修正。

### 任务预览双时区展示（2026-09-19）

- 已交付：预览优先显示所选任务时区的日期、时间和该时刻 GMT 偏移；任务时区与电脑不同则同时显示带时区名称的本地时间。一次性/固定间隔使用本地时区。格式化异常进入可重试错误，不在渲染时崩溃。
- Edge 固定本地 Asia/Shanghai、任务 Australia/Sydney，验证同一 UTC 时刻显示悉尼 11:00 GMT+11 与上海 08:00；频率转换/请求参数、迟到响应隔离和错误恢复回归通过，生产构建通过。

- 提交后 Edge 边界验收：印度 GMT+5:30 显示 05:30，洛杉矶显示前一年 12/31 16:00 同时上海显示 1/1 08:00，任务与本地同为上海时不重复显示本地行。调整测试兼容本地数字日期不补零的格式，完整频率/预览/保存流程通过，无产品修正。

### 任务运行耗时（2026-09-19）

- 已交付：运行记录摘要显示时/分/秒耗时；运行中每秒更新“已运行”，结束后依据记录的 finishedAt 固定显示“耗时”。缺失、无效或倒序时间显示未知，避免伪造零耗时；卸载/结束清理计时器。
- 格式测试覆盖运行中/结束/失败、小时、零秒及异常时间。真实调度器驱动的浏览器验收验证运行中数字增长、中断后切换为最终耗时；任务 CRUD、运行/取消、输出、重启与窄屏/深色回归通过，生产构建通过。耗时基于墙上时钟，不代表 CPU 使用时间。

- 提交后真实调度器驱动的页面验收：中断后显示值精确对应持久化起止时间，等待超过刷新周期后不再增长；重启调度器并重载页面后最终耗时保持一致。完整任务页面回归通过，无产品修正。

### 运行记录导出耗时（2026-09-19）

- 单条复制/导出和批量历史导出复用页面的耗时计算，依据已保存的开始与结束时间输出时/分/秒；缺失、无效和倒序时间明确显示“耗时未知”。运行中不生成随当前时钟变化的导出耗时，批量导出仍排除运行中记录。
- 9 项耗时与导出测试通过，覆盖完成/失败/中断、零秒、异常时间及批量记录对应关系；生产构建和真实调度器驱动的完整任务页面回归通过。构建仍有既有的大于 500 kB 分块提示。

### 新会话本地项目路径修正（2026-09-19）

- 修复新会话发送时把本地项目路径（例如 `D:\\Workspace2026\\my-agent-plantform`）作为 app-server `projectId` 发送的问题。现在本地项目仍通过 `cwd` 传递；只有明确的远端项目身份才会传 `projectId`，因此后端不会再因本地目录不存在于项目注册表而返回 `project not found`。
- 覆盖本地、远端、已移除最近项目条目和草稿重试场景；真实 app-server 验收确认错误请求失败、修正请求成功。生产构建与 7 项相关单测、会话创建 UI 验收通过。

### 对话配置按需收纳（2026-09-19）

- 对话顶部改为标题、查找和会话设置入口；渠道、协作模式、记忆、Daybreak 与目标表单进入原生弹出面板，目标默认折叠。面板不占消息布局空间，支持 Escape、外部点击关闭，同会话关闭重开保留目标草稿，切换会话重置面板。
- 查找默认不占高度，由标题栏或 Ctrl/Command+F 唤起，关闭后焦点回到入口；空查询不再显示完整历史无匹配提示。浏览器验收覆盖设置、草稿、600px 布局及发送，完整查找与会话创建回归通过。查找测试修正为多监听器通知分发并提供有效回合身份，符合当前协议。

- 提交后补充验收：390px 窗口的面板边界与滚动操作、外部点击关闭、设置内快捷键隔离、新会话目标草稿隔离通过；规划模式测试迁移到设置入口并修正通知模拟，运行中锁定、计划事件、草稿保护与按计划执行完整流程通过。无需新增产品修正。

### 会话工作区五项增量计划（2026-09-19）

顺序：环境面板 → 更多操作 → 文件与差异侧边面板 → 线程持续跟进 → 多 Agent 协作面板。每项独立验证提交推送，再补充验收；保留现有能力和非本任务改动。

- 第 1 项：按需读取会话 cwd 的 Git 状态，展示本地/工作树、分支、变更文件数、上游，提供文件/Git 入口并收纳后台命令。浏览器验证懒加载、失败清除旧状态、重试与面板导航，生产构建通过。变更统计为文件数，不冒充增删行数。

- 第 2 项：导出、审查、重命名、分叉、归档、删除、时间线及本地/远端队列收进更多会话操作，移除常驻重复工具栏。保留挂载以维持导出锁与队列草稿；标题栏与规划流程界面验收通过。

- 第 3 项：消息文件预览与编辑使用右侧非模态面板，保留聊天输入；本回合统一差异提供右侧入口，Git 原有差异面板继续复用。预览/编辑完整回归覆盖定位、脏稿关闭确认、保存失败保留与重试；生产构建通过。窄屏使用可关闭的覆盖面板。

- 第 4 项：会话更多菜单创建定期跟进，沿用原会话上下文与渠道，在同一桌面 RPC 执行；支持只读/明确允许工作区写入、暂停恢复、立即运行、取消本次、记录与删除。复用持久调度器；任务忙碌时拒绝冲突操作，原会话忙碌则本次失败、下次重试。模型明确返回完成标记后停止后续安排，无变化标记不触发任务通知。真实 app-server 原会话续跑、29 项调度/仓库/跟进测试、真实调度器驱动的界面流程及构建通过。需应用运行，不会唤醒已退出应用；完成判断来自模型回复而非独立成果评测。

- 跟进补充验收修正：跟进回合的普通会话完成通知交由任务通知策略统一处理，避免“无变化”运行被会话通知重复提醒；取消仅指向跟进拥有的回合，清理监听器与锁的测试通过。

- 第 5 项：多 Agent 侧面板按已加载协作记录生成去重、防环的子任务层级（最多 200 项），展示最新记录/查询状态、结果与失败原因。支持打开子会话、读取最新回合、停止精确回合；追加指令按运行状态选择 steer 或新回合，失败保留草稿。领域测试、界面流程、真实后端分支会话续跑及结果读取、生产构建通过。不是服务端全量任务树；未加载后代明确不推测。面板切换统一互斥，正在编辑文件时要求先保存或取消，避免丢稿或多面板挤压。

- 第 5 项提交后验收修正：子任务指令锁提升到共享命令层，关闭重开面板仍不能重复写入；失败释放锁，追加回合确认必须有有效身份，运行中 steer 必须对应预期回合。单测和界面重试通过。五项组合的环境、标题栏、文件编辑、持续跟进、协作页面回归及真实 Electron 任务保存/重启回归通过。

- 组合验收修正：环境面板隐藏时保留后台命令组件及操作锁，Git 读取仍只在打开时执行；验证关闭重开不会重复终止进程。导出入口迁移后完整分页/取消/失败/本机导出回归通过。截图检查发现并清除旧 JSX 中意外显示的字面量换行标记。
