...
计划和交付：顶部会话分叉与消息级分叉共用忙碌锁，运行中、加载中或断线时禁止重复请求；异步完成后若用户已切换会话，分支仍保存到最近会话但不会抢回当前视图。分支继承每会话模型、推理强度、规划模式、项目和工作区，清除计划进度、上下文用量、权限请求及旧权限快照，并重置为可发送的空闲状态。单元验收覆盖深拷贝、历史截断和临时状态清理；生产构建通过。
