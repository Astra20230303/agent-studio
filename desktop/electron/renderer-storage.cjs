...
const KEYS = ['codex-desktop-state-v1', 'felix-thread-drafts-v1', 'felix-attachments-v1', 'felix-plugin-drafts-v1', 'felix-skill-drafts-v1', 'felix-turn-queue-v1', 'felix-audit-log-v1', 'felix-task-editor-draft-v1'];
...
function validate(key, value) {
  ...
  } else if (key === 'felix-task-editor-draft-v1') {
    // Drafts can be incomplete. The renderer validates domain fields on recovery.
    if (!Object.values(parsed).every(entry => entry && typeof entry === 'object' && !Array.isArray(entry))) throw Error('Invalid task draft data');
